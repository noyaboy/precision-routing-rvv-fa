 ...
 * Scalar unpack of packed bytes into u16-nibble buffer; native vfconv
 * decodes nibble -> BF16; BF16 widened to FP32 + scaled. */
static inline void dequant_row_native(const uint8_t *pk_row,
    ...
    bf16_t   bf16_buf[NVFP4_BLOCK]  __attribute__((aligned(64)));
    ...
    for (int blk = 0; blk < blocks_per_row; blk++) {
        ...
        /* Native vfconv.nvfp4.bf16.v: u16 nibbles -> BF16.  Encoding
        ...
        asm volatile (
            ...
            "vse16.v   v0, (%[out])\n\t"
            :
            : [in]"r"(nibbles), [out]"r"(bf16_buf)
            : "memory", "v0"
        );
        /* BF16 widen + scale multiply. */
        float scale = e4m3_decode[sc_row[blk]];
        size_t vl = __riscv_vsetvl_e32m2(NVFP4_BLOCK);
        vfloat32m2_t v = bf16_load_widen(bf16_buf, vl);
        v = __riscv_vfmul_vf_f32m2(v, scale, vl);
        __riscv_vse32_v_f32m2(fp32_out + blk * NVFP4_BLOCK, v, vl);
    }
}
...
int main(void) {
    ...
    for (int h = 0; h < N_HEADS; h++) {
        ...
        /* BISECTION VARIANT: scalar libm expf instead of native vfexp.v. */
        float sum_p = 0.0f;
        for (int s = 0; s < SEQ_LEN; s++) {
